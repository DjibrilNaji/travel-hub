...
export async function GET(req: NextRequest, { params }: { params: Promise<Params> }) {
  ...
  if (!resolvedParams.id || resolvedParams.id.length !== 24) {
    return NextResponse.json({ error: "Missing id parameter" }, { status: StatusCodes.BAD_REQUEST })
  }
  ...
  try {
    const offer = await mongo.collection("offers").findOne({ _id: new ObjectId(resolvedParams.id) })

    if (!offer) {
      return NextResponse.json({ error: "Offer not found" }, { status: StatusCodes.NOT_FOUND })
    }
    ...
    const relatedOffers = result.records.map((record) => record.get("related._id"))
    ...
    return NextResponse.json({ offer, relatedOffers }, { status: StatusCodes.OK })
  } catch (error) {
